		...
		<div class="row flex-nowrap">
			...
			<div class="col py-3">
				...
				<h2 class="text-center">Create Result</h2>
					<form  role="search" action="/resultinfo" class="shadow p-4">
						<label for="id" class="form-label">Select Semester</label> <select
							...
							<c:forEach var="d" items="${se}">
								<option value="${d.semestername }">${d.semestername }</option>
							</c:forEach>

						</select>
						<label for="id" class="form-label">Select Course</label> <select
							name="coursename" class="form-select mb-3"
							aria-label="Default select example" id="id">
							<c:forEach var="s" items="${co}">
								<option value="${s.coursename }">${s.coursename }</option>
							</c:forEach>
